...
const BotonDeEnvio = ({ canSubmit, isSubmitting, texto }: BotonDeEnvioProps) => {
  return (
    <button
      ...
      {isSubmitting ? (
        // Muestra solo el spinner si se está enviando
        // El tamaño h-6 w-6 ocupa el espacio vertical del texto y el padding
        <Loader2 className='h-6 w-6 animate-spin' />
      ) : (
        // Muestra el texto normal cuando no hay envío
        texto
      )}
    </button>
  );
